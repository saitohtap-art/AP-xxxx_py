...
print('AP-4370用サンプルプログラムです。')
print('プリンタのショリモードをRFに設定し使用してください。')
print('機器の通信設定は115200bps,8N1に設定してください。')
...
I_COMM = b'I\r'
F_COMM = b'FU0008\r'   #自動RFID読取の場合は b'AU0008¥r'、QR読取は b'F¥r'

ESC = b'\x1B'
LFNUL = b'\x0A\x00'
#印刷データ
PrintData = [ESC + b'D0856,1500,0750' + LFNUL]
PrintData += [ESC + b'C' + LFNUL]
PrintData += [ESC + b'LC;0300,0000,1250,0510,1,5' + LFNUL]
PrintData += [ESC + b'LC;0300,0170,1250,0170,0,3' + LFNUL]
PrintData += [ESC + b'LC;0300,0340,1250,0340,0,3' + LFNUL]
PrintData += [ESC + b'LC;0490,0000,0490,0510,0,3' + LFNUL]
PrintData += [ESC + b'PC00;0320,0110,1,1,X,00,B' + LFNUL]
PrintData += [ESC + b'PC01;0320,0280,15,2,V,00,B' + LFNUL]
PrintData += [ESC + b'PC02;0320,0440,1,1,X,00,B' + LFNUL]
PrintData += [ESC + b'PC03;0510,0150,2,4,W,00,B' + LFNUL]
PrintData += [ESC + b'PC04;0510,0490,3,5,W,00,B' + LFNUL]
PrintData += [ESC + b'PV01;0510,0320,0100,0150,B,00,B' + LFNUL]
PrintData += [ESC + b'XB00;0700,0540,T,M,06,A,0,M2' + LFNUL]
...
PrintData += [ESC + b'RC03;' + 'エイピーリファイン'.encode('shift_jis') + LFNUL]
PrintData += [ESC + b'RC04;' + 'AP-4370'.encode('shift_jis') + LFNUL]
PrintData += [ESC + b'RV01;0566915001' + LFNUL]
PrintData += [ESC + b'RB00;1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890' + LFNUL]
PrintData += [ESC + b'XS;I,0001,0000CC000' + LFNUL]
#白紙印刷データ
BlankData = [ESC + b'D0856,1500,0750' + LFNUL + ESC + b'C' + LFNUL + ESC + b'XS;I,0001,0000CC000' + LFNUL]
    ...
    if data_len >= 0:
        if data_rcv[data_len] == ord('\r'):  #受信データ最後 CR 確認
            ...
            if data_len >= 2 and data_rcv[0] == 0x1b:   #ESC
                ...
                #RFID カウンタ値の読み取り
                resultData = rfidStat(b'FU0001\r')  #RFID 00ブロック読み取り
                if len(resultData) >= 4:
                    try:
